...
## HMRC VAT Authorisation
  ...
  *automatically* issue a first-time authorisation request.  Otherwise re-authorisation will be manual - unless the automatic_reauthorisation_requests flag is set to true, in which case it's the dashboard that does the work.
  ...
  ### REQUEST_HMRC_VAT_AUTHORISATION + HMRC_VAT_AUTHORISATION_REQUESTED
  
  This is a Tenancy event.  Suggested attributes:
  
  - creation date/time
  - UUID
  
  I don't think we need much more on it.
  
  When this event is applied to the tenany it will set a pendingAuthorisation flag to true.  When the subsequent HMRC_VAT_AUTHORISED event is received it will set this flag back to false.
  
  When we detect that a token has expired (or is missing) we can see if there is a pendingAuthorisation on the tenancy and whether or not it has expired.  This 
  will help us to decide whether to send out another request.
  
  We can implement a method on the tenancy to determine whether the pendingAuthorisation has expired - creation time + expiry window > current time.
  
  The notification service will subscribe to this event and send an email.  The user will click on a link that leads to an Authorise page.  The Authorise page implements the HMRC redirection and comes back to an authorised page that generates the next command
  
  ### AUTHORISE_HMRC_VAT + HMRC_VAT_AUTHORISED
  
  This is also a tenancy event that just gets persisted and published
  - authorised date/time (same as the event date/time)
  - auth code details!?

  The notification service should subscribe and send a confirmation email (nice-to-have)
  
  The HMRC service will subscribe and exchange the auth code for an access token.  This token will NOT be persisted here...instead it will be published as...
  
  ### CAPTURE_HMRC_ACCESS_TOKEN + HMRC_ACCESS_TOKEN_CAPTURED
  
  The tenancy service will subscribe to this and store the access token
  
  ## Dashboard workflow
  
  When the dashflow attempts to retrieve VAT info it might need the access token to be refreshed.  If this is the case it will directly call the HMRC service to request a new token REFRESH_HMRC_VAT_TOKEN.  The HMRC service will publish the token it creates (HMRC_VAT_TOKEN_REFRESHED), and also return it back to the dashboard.  The dashboard will use it.  We need to find a way to make this synchronous!  We might have to go into a retry loop - at least as a temporary measure.
